...
if (application._enableaccessibility && nexacro._accessibilitytype != application._accessibilitytype) {
...
if (nexacro._accessibilitytype == 2) {
	...
	nexacro.AccessibilityUtil.supportMobileApplicationAccessibility = function (container_handle, bForce) {
		...
		if (container_element && !bForce) {
			...
			trace(comp.name, comp._accessibility_role);
			if ((comp && comp._is_component && !comp._is_subcontrol && !(comp._is_form && comp.parent._is_frame) && !comp._is_frame)) {
				if (comp._accessibility_role != "spin" && comp._accessibility_role != "grid" && comp._accessibility_role != "treegrid" && comp._accessibility_role != "radio"
					 && comp._accessibility_role != "textbox" && comp._accessibility_role != "edit" && comp._accessibility_role != "webbrowser" && comp._accessibility_role != "calendar" && comp._accessibility_role != "form") {
					nexacro.__setDOMNodeAccessibilityHidden(container_handle, true);
				}

				if ((comp._is_form && !comp.parent._is_frame) && comp.components.length <= 0) {
					nexacro.__setDOMNodeAccessibilityHidden(container_handle, true);
				}
			}
	...
	nexacro.AccessibilityUtil.supportMobileApplicationAccessibility = function (control_element, bForce) {
		return;
		if (control_element && !bForce) {
			var comp = control_element.linkedcontrol;

			if ((comp && comp._is_component && !comp._is_subcontrol && !comp._is_form && !comp._is_frame)) {
				nexacro.__appendElementHandleAccessibilityNodeList(control_element._handle);
			}
			else if (comp && (comp instanceof nexacro.ScrollBar)) {
			}
		}
	...
	nexacro.AccessibilityUtil.unsupportMobileApplicationAccessibility = function (control_element, bForce) {
		...
		if (control_element && !bForce) {
			...
			if ((comp && comp._is_component && !comp._is_subcontrol && !comp._is_form && !comp._is_frame)) {
				nexacro.__removeElementHandleAccessibilityNodeList(control_element._handle);
			}
			else if (comp && (comp instanceof nexacro.ScrollBar)) {
			}
		}
	...
	nexacro.AccessibilityUtil.getAccessibilityAdditionalLabel = function (elem) {
		...
		if (elem) {
			...
			if (comp && (comp._has_inputElement || comp._input_element)) {
				...
				if (comp._is_form) {
					if (comp._text_elem) {
						strAdditionalLabel = comp._display_text;
					}
				}
			}
		}
